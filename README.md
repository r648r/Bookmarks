...
### Raphdan.js - Collecteur de Hostnames

Use shodan cli it's better
...
javascript:(function(){
  ...
  if(0===e.length)return void alert("Aucun élément trouvé avec le sélecteur sur cette page");
  ...
  o.document.write(`<html><head><title>raphdan - Hostnames (${a.length})</title><style>body{font-family:monospace;background:#f0f0f0}pre{background:white;padding:10px;border:1px solid #ccc}.info{color:#800020;margin-bottom:10px}button{background:#ff4136;color:white;border:none;padding:8px 12px;cursor:pointer;margin-bottom:10px}</style></head><body><div class="info"><b>${t.length}</b> hostnames trouvés sur cette page, <b>${a.length}</b> hostnames au total stockés.</div><button onclick="if(confirm('Voulez-vous vraiment supprimer les '+${a.length}+' hostnames sauvegardés ?')){localStorage.removeItem('raphdan_hostnames');alert('Liste de hostnames supprimée avec succès!');window.close();}">Supprimer tous les hostnames</button><pre>${a.join("\n")}</pre></body></html>`),
  o.document.close()
})();
```
